...
DGCG.config.frequencies_buffer = DGCG.opencl_mod.create_buffer(FREQUENCIES,
                                                               flag='WRITE')
...
""" Testing that the test function implementations are proper. Also,
testing if the acceleration is adequate. 

For N = 100, and T = 51, got 1500 times faster calculation for the gradient
"""

N = 1
x = np.random.rand(N,2)
#times = np.arange(51).astype(int)
#T = len(times)


#x = np.array([0.5, 0.5])
times = np.array([7, 4, 10]).astype(np.int32)
T = len(times)
K = DGCG.config.K
# Initialize buffers
x_buff = DGCG.opencl_mod.create_buffer(x, flag='COPY')
t_buff = DGCG.opencl_mod.create_buffer(times, flag='COPY')

out_real_buff = DGCG.opencl_mod.create_buffer(np.zeros([T, N, K]), flag='WRITE')
out_imag_buff = DGCG.opencl_mod.create_buffer(np.zeros([T, N, K]), flag='WRITE')
# Apply function
DGCG.opencl_mod.test_func(x_buff, t_buff, out_real_buff, out_imag_buff, N, T)

print("Observe results")
print(DGCG.opencl_mod.extract_buffer(out_real_buff, [T, N, K]))
print(DGCG.opencl_mod.extract_buffer(out_imag_buff, [T, N, K]))
...
x = np.array([[0.01, 0.01]]).astype(np.float64)
t = 15
t_samples = np.array([t])
test = DGCG.opencl_mod.TEST_FUNC(1)
test.load(x, t_samples)
test.run(times=exec_times)
res = test.unload()
...
print(f"Non-parallelized execution time {end_time/exec_times}")

diff_1 = res[0] - np.real(res2)
diff_2 = res[1] - np.imag(res2)

print("Differences")
print(np.linalg.norm(diff_1)/np.linalg.norm(res[0]))
print(np.linalg.norm(diff_2)/np.linalg.norm(res[1]))
print(f"Norm pieces {np.linalg.norm(res[0])}")
